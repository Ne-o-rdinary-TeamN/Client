...
import HotNewsAnimatedWrapper from "./HotNewsAnimatedWrapper";

function HotNewsButton({
  index,
  title,
  link,
}: {
  ...
  return (
    <Link
      href={link}
      className="grid grid-cols-10 items-center gap-2 bg-white rounded-xl justify-between py-2 px-4 h-[48px] box-border"
    >
      ...
      <div className="flex flex-col gap-1.5 text-gray-006 font-regular-12 mt-3">
        {newsItems.length > 0 ? (
          newsItems.map((news, index) => (
            <HotNewsAnimatedWrapper index={index} key={index}>
              <HotNewsButton
                key={index}
                index={index + 1}
                title={news?.title}
