...
export const fetchRegisterPostClient = async (
  page: number = 0
  ...
  const response = await fetch(
    `${PUBLIC_API_URL}${ENDPOINT.POST.READ_CREATED}?${params}`,
    {
      headers: {
  ...
  const data: BaseResponse<RegisterPostResponse> = await response.json();
