...
import type {
  ...
  RegisterPostDTO,
} from "../model/registerPost";

const convertToPost = (dto: RegisterPostDTO): Post => {
  return {
    title: dto.postName,
...
export const fetchRegisterPost = async (userPk: number, page: number = 0) => {
  const response = await Http.get<BaseResponse<RegisterPostResponse>>({
    request: ENDPOINT.POST.READ_CREATED,
    authorize: true,
    params: {
      page,
    },
    next: {
      revalidate: 60 * 2,
      tags: [REVALIDATE_TAG.POST_CREATED(userPk)],
    },
  });
