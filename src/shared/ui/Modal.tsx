...
export default function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div
        className={`relative bg-white rounded-2xl p-6 w-[90%] max-w-[320px] shadow-lg transition-all duration-300 ${isAnimating
          ? 'opacity-100 scale-100'
          : 'opacity-0 scale-95'
          }`}
      >
        <h2 className="font-bold-20 text-gray-007 mb-3">{title}</h2>
        <p className="font-regular-16 text-gray-006 mb-6 whitespace-pre-line">{message}</p>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-xl font-semibold-14 ${variant === 'danger'
